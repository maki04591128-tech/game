...
### 10.15 ドキュメントブラッシュアップ（第13回）

> エージェント提案アイデア7件のうち4件をユーザーが採用確定。
> 採用: 天候(§19.1)/罠作成(§19.2)/料理(§19.4)/評判名声(§19.5)。不採用: 変身(§19.3)/夢幻覚(§19.6)/遺産遺跡(§19.7)。
> 採用確定システムの優先度引き上げ・ロードマップ反映。

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| D.112 | 21_拡張システム設計書 §19 採否ステータス追記 | ✅ | 各提案見出しに✅採用/❌不採用マーク追加。冒頭に採否一覧表追加 |
| D.113 | 実装計画書Ver.prt.0.2 採用4件の優先度引き上げ | ✅ | P.18天候:中→高、P.40罠作成:低→中、P.29料理:低→中、P.42評判名声:中→高。各行に✅採用マーク |
| D.114 | 実装計画書Ver.prt.0.2 不採用3件のマーキング | ✅ | P.41変身/P.43夢幻覚/P.44遺産遺跡を取り消し線+❌不採用マーク |
| D.115 | 実装計画書Ver.prt.0.2 ロードマップ更新 | ✅ | 採用4件をロードマップに組み込み（P.42→0.2、P.18→0.3、P.40/P.29→1.0）。不採用3件を除外し不採用注記追加 |
| D.116 | 実装計画書Ver.prt.0.1 ブラッシュアップ第13回記録追加 | ✅ | 本セクション |

### 10.16 ドキュメントブラッシュアップ（第14回）

> ユーザー追加提案13件の新システムアイデアを拡張システム設計書・実装計画書に追加。
> 攻撃属性/アイテム等級/拠点作成/採取/釣り/剥ぎ取り/物品価値変動/描画最適化/
> 時刻行動変化/シンボルマップイベント/ダンジョン特徴生成/勢力地変動/グリッドインベントリ。

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| D.117 | 21_拡張システム設計書 §25-§37 追加 | ✅ | 13件の新システム設計（概要/参考作品/設計方針/既存関連/対応Ver）。目次更新。依存関係マップ更新。改訂履歴追加 |
| D.118 | 実装計画書Ver.prt.0.2 P.45-P.57 追加 | ✅ | 13件をタスク候補一覧の各カテゴリに振り分け。戦闘(P.45)/装備(P.46)/AI(P.52,P.53)/世界(P.54,P.55,P.56)/サバイバル(P.47,P.48,P.49,P.50)/UI(P.57)/その他(P.51) |
| D.119 | 実装計画書Ver.prt.0.2 ロードマップ更新 | ✅ | 13件を4段階ロードマップに配置。Ver.prt.0.2に5件(P.45/P.46/P.50/P.53/P.55)、0.3に8件(P.48/P.49/P.51/P.52/P.54/P.56)、1.0に3件(P.47/P.57) |
| D.120 | 実装計画書Ver.prt.0.1 ブラッシュアップ第14回記録追加 | ✅ | 本セクション |

### 10.17 ドキュメントブラッシュアップ（第15回）

> エージェント提案第2回：類似ゲームから参考となるアイデア40件を提案し、ユーザーが27件を採用。
> 採用27件を§38-§64/P.58-P.84として拡張システム設計書・実装計画書に追加。
> 不採用13件: 部位狙い/リアクション/陣形/刻印/突然変異/遺伝血統/称号/師弟/光源/永続世界変化/過去の自分/ナレーション/チャレンジモード。

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| D.121 | 21_拡張システム設計書 §38-§64 追加 | ✅ | 27件の新システム設計（概要/参考作品/設計方針/既存関連/対応Ver）。目次27項目追加。エージェント提案第2回採否一覧追加。依存関係マップ更新。改訂履歴第4版追加 |
| D.122 | 実装計画書Ver.prt.0.2 P.58-P.84 追加 | ✅ | 27件をタスク候補一覧の各カテゴリに振り分け。戦闘(P.58-P.60)/成長(P.66,P.67)/AI(P.62,P.65)/世界(P.61,P.63,P.64,P.75)/NPC(P.68-P.70)/サバイバル(P.71-P.73)/UI(P.74,P.79-P.81)/その他(P.76-P.78,P.82-P.84) |
| D.123 | 実装計画書Ver.prt.0.2 ロードマップ更新 | ✅ | 27件を4段階ロードマップに配置。Ver.prt.0.3に12件、1.0に13件、2.0に2件（テキスト/音響依存） |
| D.124 | 実装計画書Ver.prt.0.1 ブラッシュアップ第15回記録追加 | ✅ | 本セクション |

### 10.18 ドキュメントブラッシュアップ（第16回）

> エージェント提案第2回の採用（§38-§64/P.58-P.84）追加完了後の全ドキュメント整合性チェック。
> 00_ドキュメント概要の拡張システム数を最新化（71システム）。ロードマップ依存関係の注記追加。

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| D.125 | 00_ドキュメント概要 拡張システム設計書の説明更新 | ✅ | 「24システム＋提案7システム」→「§1-§64計71システム（ユーザー37+エージェント第1回7+第2回27）。P.84候補」に修正。セクション4.5の説明も更新 |
| D.126 | 実装計画書Ver.prt.0.2 ロードマップ依存関係注記追加 | ✅ | P.71/P.72（0.3予定）がP.28（1.0予定）に依存する件について、0.3では簡易版先行実装→1.0で正式統合の方針を注記 |
| D.127 | 実装計画書Ver.prt.0.1 ブラッシュアップ第16回記録追加 | ✅ | 本セクション |

### 10.19 ドキュメントブラッシュアップ（第17回）

> Ver.prt.0.2-0.4実装完了後のドキュメント全体漏れ補完。
> テスト数・ファイル数・検証項目数・キーバインド・画面遷移・クラス一覧を最新の実装状態に整合。

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| D.138a | 17_デバッグ・テスト設計書 全面更新 | ✅ | テスト数980→2061件、ファイル数19→87。テスト一覧に68ファイル追加。GUIオート37→38検証項目、システム検証37→24検証項目。実装状況1119→2200件 |
| D.138b | 13_GUIシステム設計書 キーバインド追加 | ✅ | Y=図鑑/U=仲間/Z=死亡録の3キーバインドをテーブルに追加 |
| D.138c | 20_GUI画面遷移図 ウィンドウ追加 | ✅ | 新ダイアログ6種（図鑑/死亡録/スキルツリー/仲間/料理/拠点管理）追加。ウィンドウ数18→24 |
| D.138d | 10_プロジェクト構造設計書 更新 | ✅ | テスト数980→2061件、Systems/説明をCharacterCreation→77ファイルに更新 |
| D.138e | 11_クラス設計書 実装状況追加 | ✅ | Ver.prt.0.2-0.4の新システムクラス14件を実装状況テーブルに追加 |
| D.138f | 00_ドキュメント概要 更新 | ✅ | 20_GUI画面遷移図の説明を「全24ウィンドウ」に更新 |
| D.138g | 実装計画書 ブラッシュアップ記録 | ✅ | 本セクション + Ver.prt.0.2にD.138追加 |
