# 実装計画書 Ver.prt.0.25 — 満腹度・渇き度システム全面改修

**ステータス**: ⬜ 未着手
**目的**: 満腹度・渇き度システムを全面改修し、段階的なペナルティ・行動コスト加算・過食/飲み過ぎ状態を含む10段階制に拡張する
...
Phase 6: UI更新（タスク 23-24）
  ↓
Phase 7: テスト（タスク 25-30）
```

**注意**: Phase 1〜3 は下位層から変更するため、各Phase完了後にビルド確認を行うこと。Phase 4 以降はGameController の大規模変更となるため、タスク単位でのビルド・テスト確認を推奨する。
...
| `src/RougelikeGame.Engine/TurnSystem/TurnManager.cs` | 行動コスト統合 | 中 |
| `src/RougelikeGame.Gui/GameController.cs` | 消費ロジック・ダメージ・制限 | 高 |
