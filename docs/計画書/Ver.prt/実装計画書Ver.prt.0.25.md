...
| 90以上100未満 | 疲弊 | −80% | +4 | 移動・攻撃・スキル使用不能 | 基本回復量×1/1000 | — |
| 100 | 疲労困憊 | −100% | +5 | 待機・インベントリ操作・アイテム使用のみ可能 | 基本回復量×1/1000 | — |
...
- **疲弊状態**（90〜99）: 移動・攻撃・スキル使用不能。SP上限−80%、全行動コスト+4。待機でわずかに回復可能
- **疲労困憊状態**（100）: 待機・インベントリ操作・アイテム使用のみ可能。SP上限−100%（実質SP0）、全行動コスト+5
...
| 宿屋宿泊 | 疲労度を0にリセット | 疲労状態(70+): 10から開始、疲労(重)(80+): 20から開始、疲弊以上(90+): 宿屋利用不可(要検討) |
