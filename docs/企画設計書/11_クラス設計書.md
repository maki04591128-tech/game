...
> | Player | ✅ 基本実装済み | `Race`/`CharacterClass`/`Background` は未実装。スキルは `HashSet<string>`、魔法言語は `Dictionary<string,int>`、宗教は `string?`+`int` で簡略化 |
...
> | TransferData | ⬜ 未実装 | 死に戻り引き継ぎは Phase 5 以降で実装予定 |
>
> **実装コードの参照先**: `src/RougelikeGame.Core/Entities/`, `src/RougelikeGame.Core/ValueObjects/`
