...
| クラス名 | テスト数 | 目的 | ファイル |
|---|---|---|---|
| `GuiAutomationTests` | 37件 | UI要素の存在・基本操作の検証 | `GuiAutomationTests.cs` |
| `GuiSystemVerificationTests` | 37件 | Ver.prt.0.1で実装した各システムのGUI上での動作検証 | `GuiSystemVerificationTests.cs` |
| **合計** | **74件** | | |
...
**用途**: メインウィンドウ・キー操作・メッセージログのテスト（30件）
...
### GuiAutomationTests（37件）

| カテゴリ | テスト数 | 起動方式 | 平均所要時間 |
|---|---|---|---|
| タイトル画面 | 4 | 通常起動 | 約1〜3秒/件 |
| 設定画面 | 3 | 通常起動 | 約3秒/件 |
| メインウィンドウUI | 16 | デバッグマップ | 約2秒/件 |
| キー操作 | 12 | デバッグマップ | 約2〜7秒/件 |
| メッセージログ | 2 | デバッグマップ | 約2秒/件 |
| **小計** | **37** | — | **約2分10秒** |

### GuiSystemVerificationTests（37件）

| カテゴリ | テスト数 | 起動方式 | 平均所要時間 |
|---|---|---|---|
| Phase 1-2: 基盤・コアシステム | 4 | デバッグマップ | 約2〜7秒/件 |
| Phase 3: GUI・操作 | 2 | デバッグマップ | 約2〜3秒/件 |
| Phase 4: ゲームプレイ拡張 | 18 | デバッグマップ | 約2〜12秒/件 |
| Phase 5: コンテンツシステム | 2 | デバッグマップ | 約2〜4秒/件 |
| Phase 6: リリース準備基盤 | 3 | 通常起動/デバッグマップ | 約3〜5秒/件 |
| 統合テスト | 4 | デバッグマップ | 約5〜10秒/件 |
| その他: 表示フォーマット | 4 | デバッグマップ | 約2秒/件 |
| **小計** | **37** | — | **約2分53秒** |
