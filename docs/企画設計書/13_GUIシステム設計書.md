...
| 項目 | フェーズ | 説明 |
|------|---------|------|
| タイトル画面 | Phase 6 | スタート/ロード/設定メニュー |
| キャラクター作成画面 | Phase 4 | 種族・職業選択、ステータス振り分け |
| セーブ・ロード画面 | Phase 4 | JSON形式でのゲーム状態保存・復元 |
| メッセージログ履歴 | Phase 4 | ログ履歴閲覧、フィルタリング |
| 設定画面 | Phase 6 | キー設定、音量調整 |
| アニメーション | Phase 6 | 攻撃エフェクト、画面遷移 |
