...
│  │  真のゲームオーバー → MessageBox → MainWindow終了          │       │
│  │                                    → アプリ終了            │       │
│  │                                                          │       │
│  └──────────────────────────────────────────────────────────┘       │
└─────────────────────────────────────────────────────────────────────┘
...
App.Application_Startup
  │
  ├─ --skip-title 引数あり → MainWindow（ニューゲーム即開始、デフォルト設定）
  ...
  └─ 通常起動 → TitleWindow.ShowDialog()
       ...
       │    │
       │    └─ ❸ App.StartMainWindow() → MainWindow起動
       │         │
...
| 23 | CookingWindow | CookingWindow.xaml/.cs | MainWindow / TownWindow | Hキー（料理モード選択時） |
