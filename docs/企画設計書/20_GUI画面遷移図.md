...
│  │  E → SkillTreeWindow（スキルツリー・習得済みタブ）              │      │
│  │  1-6 → スキルスロット使用（割当済スキル発動）           │      │
│  │  ※ H（旧鍛冶）/ B（旧街入場）はキー削除済み            │      │
│  │  M → ミニマップ表示/非表示（画面内トグル）                   │      │
│  │  F5 → セーブ実行（ダイアログなし）                          │      │
│  │  F9 → ロード実行（ダイアログなし）                          │      │
│  │  Q → ゲーム終了                                            │      │
│  │  Esc → PauseWindow（ポーズ画面）                          │      │
│  │    → 再開 / セーブ / ロード / 設定 / キーバインド / タイトルへ │      │
│  │    → 設定 → SettingsWindow → PauseWindowに戻る          │      │
│  │    → キーバインド → KeyBindingWindow → PauseWindowに戻る  │      │
│  │    → タイトルへ → TitleWindow                              │      │
│  │                                                           │      │
│  └───────────────────────────────────────────────────────────┘      │
│                                                                     │
...
│  │  ※ 全ロケーション入場はTキー（TryEnterTown）に統一      │       │
│  │  建物入口(BuildingEntrance) → 建物内部マップに遷移          │       │
│  │  建物出口(BuildingExit) → 町マップに復帰                    │       │
│  │                                                          │       │
│  └──────────────────────────────────────────────────────────┘       │
└─────────────────────────────────────────────────────────────────────┘
```
...
[TownWindow]
    ...
    │
    └─ ダンジョン入場 ───→ TownWindow閉じる → MainWindow（Tキー入場処理）
```
...
| 21 | SkillTreeWindow | SkillTreeWindow.xaml/.cs | MainWindow | Eキー（6タブ：種族/職業/素性/武器/魔法/習得済み） |
